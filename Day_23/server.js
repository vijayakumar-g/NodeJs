...
db.collection("Storage").find({Name:req.body.Name}).toArray(function(err,data)
{
  if(!err)
  if(data.length!=0)
  {
  ...
  {
    db.collection("Storage").insertOne({Name:req.body.Name,Password:req.body.Password},function(err,result)
      {
          if(err)throw err;
      });
      res.json({data:"true"});
...
db.collection("Storage").find({Name:req.body.Name}).toArray(function(err,data)
{
  ...
  if(data.length!=0)
  {
    if(data[0].Password==req.body.Password)
    {
      ...
      res.json({data:"false",name:req.body.Name});
    }
    else
    {
    res.json({data:"falsepwd",name:req.body.Name});
    }
  }
  else
  {
    res.json({data:"true",name:req.body.Name});
  }
});
});
...
{
db.collection(data.user).insertOne(data);
